...
it('documents the complete resumable local video production workflow', () => {
  const workflow = sectionBetween('## Local Video Production', '## Movie Quote Montage')
  ...
  expect(workflow).toContain('Get-Content -Raw artifacts/latest-plan.json | ConvertFrom-Json')
  expect(workflow).toContain('npm run video -- resume --manifest $active.manifestPath')
  expect(workflow).toContain('`video_render_jobs`')
  expect(workflow).toContain('`video_asset_downloads`')
  expect(workflow).toContain('`video_render_segments`')
  ...
  expect(workflow).toMatch(/start[^.]+existing status[^.]+before[^.]+download-info preflight/i)
  expect(workflow).toMatch(/existing completed[^.]+zero Vecteezy provider calls/i)
  expect(workflow).toMatch(/preflight rejection[^.]+editable review[^.]+replace[^.]+candidate[^.]+produce/i)
  ...
  expect(workflow).toMatch(/exact quote[^.]+never[^.]+rewritten/i)
  expect(workflow).toMatch(/no dialogue[^.]+Ollama/i)
  expect(workflow).toMatch(/FFmpeg[^.]+ffprobe/i)
  expect(workflow).toMatch(/rollback and deactivation/i)
})
